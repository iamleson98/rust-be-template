//! Post service — CRUD operations on posts with RBAC enforcement.
//!
//! Holds its dependencies directly. Constructed once at startup and
//! stored as `Arc<PostService>` on `AppState`.
...
impl PostService {
    ...
    pub async fn create(
        ...
        Ok(self.store.create_post(caller_id, title, body).await?)
    }
    ...
    pub async fn update(
        ...
        self.rbac
            ...
            .await
            .map_err(AppError::from)?;
        Ok(self.store.update_post(id, title, body).await?)
    }

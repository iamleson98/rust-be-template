...
pub trait ChatStore: Send + Sync {
    ...
    async fn list_channels(&self, user_id: &str, limit: u64) -> StoreResult<Vec<chat_channel::Model>>;
    ...
    async fn message_exists_by_client_id(&self, channel_id: &str, client_msg_id: &str) -> StoreResult<bool>;
    async fn insert_zeroclaw_exchange(&self, ex: NewZeroClawExchange) -> StoreResult<()>;
    async fn list_zeroclaw_exchanges(&self, limit: u64, offset: u64) -> StoreResult<Vec<zero_claw_exchange::Model>>;
...
impl ChatStore for DbChatStore {
    async fn channel_exists(&self, channel_id: &str) -> StoreResult<bool> {
        ...
        Ok(chat_channel::Entity::find_by_id(uuid).one(self.db.as_ref()).await?.is_some())
    }

    async fn get_channel(&self, channel_id: &str) -> StoreResult<Option<chat_channel::Model>> {
        let uuid = Uuid::parse_str(channel_id)
            .map_err(|_| StoreError::Validation(format!("invalid channel id: {channel_id}")))?;
        Ok(chat_channel::Entity::find_by_id(uuid).one(self.db.as_ref()).await?)
    }
    ...
    async fn create_channel(
        ...
        chat_channel::Entity::insert(model).exec_without_returning(self.db.as_ref()).await?;
    ...
    async fn update_channel_preview(&self, channel_id: &str, preview: String, at: String) -> StoreResult<()> {
        let uuid = Uuid::parse_str(channel_id)
            .map_err(|_| StoreError::Validation(format!("invalid channel id: {channel_id}")))?;
    ...
    async fn list_messages(&self, channel_id: &str, limit: u64, offset: u64) -> StoreResult<Vec<chat_message::Model>> {
    ...
    async fn insert_message(&self, msg: NewChatMessage) -> StoreResult<chat_message::Model> {
        ...
        chat_message::Entity::insert(model).exec_without_returning(self.db.as_ref()).await?;
    ...
    async fn insert_zeroclaw_exchange(&self, ex: NewZeroClawExchange) -> StoreResult<()> {
        ...
        zero_claw_exchange::Entity::insert(model).exec_without_returning(self.db.as_ref()).await?;
        Ok(())
    }
...
impl<S: ChatStore> ChatStore for CacheChatStore<S> {
    ...
    async fn update_channel_preview(&self, channel_id: &str, preview: String, at: String) -> StoreResult<()> {
        let res = self.inner.update_channel_preview(channel_id, preview, at).await;
        if res.is_ok() {
    ...
    async fn message_exists_by_client_id(&self, channel_id: &str, client_msg_id: &str) -> StoreResult<bool> {
        self.inner.message_exists_by_client_id(channel_id, client_msg_id).await
    }

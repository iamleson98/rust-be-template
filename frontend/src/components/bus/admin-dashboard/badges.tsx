...
import { CUSTOMER_SEGMENTS } from './mock-data'
...
/* (moved here from the main file because it shares the CUSTOMER_SEGMENTS
    type and is only used by the stats-overview panel) */

export function SegmentationDonut({
  segments,
  total,
}: {
  segments: typeof CUSTOMER_SEGMENTS
  total: number
}) {
  ...
  const pcts = segments.map((s) => s.count / total)
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          {active ? (
            ...
            <>
              ...
              <div className="text-[10px] text-muted-foreground">Tổng khách hàng</div>
            </>
          )}
        </div>
      </div>

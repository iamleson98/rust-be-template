...
export const PROVIDER_OPTIONS: { value: string; label?: string; labelKey?: string; icon: React.ReactNode; color: string }[] = [
  { value: 'all', labelKey: 'common.all', icon: <Filter className="h-3.5 w-3.5" />, color: 'text-muted-foreground' },
  ...
  { value: 'cod', labelKey: 'adminPayments.providerCod', icon: <Banknote className="h-3.5 w-3.5" />, color: 'text-amber-600' },
]
...
export function StatusBadge({ status }: { status: PaymentStatus | string }) {
  ...
  const map: Record<PaymentStatus, { label: string; cls: string; icon: React.ReactNode }> = {
    pending: { label: t('admin.stats.openCount'), cls: 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/30 dark:text-amber-400 dark:border-amber-900', icon: <Clock className="h-3 w-3" /> },
    completed: { label: t('booking.complete'), cls: 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/30 dark:text-emerald-400 dark:border-emerald-900', icon: <CheckCircle2 className="h-3 w-3" /> },
    failed: { label: t('adminPayments.statusFailed'), cls: 'bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-950/30 dark:text-rose-400 dark:border-rose-900', icon: <XCircle className="h-3 w-3" /> },
    cancelled: { label: t('adminPayments.statusCancelled'), cls: 'bg-slate-50 text-slate-600 border-slate-200 dark:bg-slate-900/50 dark:text-slate-400 dark:border-slate-800', icon: <Ban className="h-3 w-3" /> },
    refunded: { label: t('adminPayments.refund'), cls: 'bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-950/30 dark:text-blue-400 dark:border-blue-900', icon: <ArrowLeftRight className="h-3 w-3" /> },
  }

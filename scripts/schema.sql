-- rustqlite schema dump (RSQLDB04, page_size=4096, pages=206, user_version=0)
...
CREATE TABLE "push_device" ( "id" uuid_text NOT NULL PRIMARY KEY, "user_id" uuid_text NOT NULL, "token" text NOT NULL, "platform" varchar(10) NOT NULL DEFAULT 'android', "created_at" text NOT NULL, "updated_at" text NULL, "last_seen_at" text NULL, FOREIGN KEY ("user_id") REFERENCES "user" ("id") ON DELETE CASCADE ON UPDATE CASCADE );
...
CREATE TABLE "route_picture" ( "id" uuid_text NOT NULL PRIMARY KEY, "route_id" uuid_text NOT NULL, "sort_order" integer NOT NULL DEFAULT 0, "storage_key" text NOT NULL, "thumb_key" text NOT NULL, "content_hash" varchar(64) NOT NULL, "mime_type" varchar(30) NOT NULL, "size_bytes" bigint NOT NULL, "thumb_bytes" bigint NOT NULL, "width" integer NOT NULL, "height" integer NOT NULL, "alt_text" varchar(255) NULL, "created_at" text NOT NULL, "updated_at" text NOT NULL, FOREIGN KEY ("route_id") REFERENCES "route" ("id") ON DELETE CASCADE ON UPDATE CASCADE );
...
CREATE TABLE "staff_presence_state" ( "user_id" uuid_text NOT NULL PRIMARY KEY, "name" text NOT NULL, "role" varchar(20) NOT NULL, "brand_id" uuid_text NULL, "online" integer NOT NULL DEFAULT 0, "last_seen_at" text NOT NULL, "last_online_at" text NULL, FOREIGN KEY ("user_id") REFERENCES "user" ("id") ON DELETE CASCADE ON UPDATE CASCADE );
...
-- index sqlite_autoindex_address_1: implicit (no stored SQL)
...
-- index sqlite_autoindex_audit_log_1: implicit (no stored SQL)
...
-- index sqlite_autoindex_booking_1: implicit (no stored SQL)
-- index sqlite_autoindex_booking_2: implicit (no stored SQL)
...
-- index sqlite_autoindex_booking_seat_1: implicit (no stored SQL)
-- index sqlite_autoindex_brand_1: implicit (no stored SQL)
-- index sqlite_autoindex_brand_2: implicit (no stored SQL)
CREATE INDEX "BusLayout_brandId_idx" ON "bus_layout" ("brand_id");

-- index sqlite_autoindex_bus_layout_1: implicit (no stored SQL)
...
-- index sqlite_autoindex_campaign_1: implicit (no stored SQL)
-- index sqlite_autoindex_campaign_2: implicit (no stored SQL)
...
-- index sqlite_autoindex_chat_assignment_1: implicit (no stored SQL)
...
-- index sqlite_autoindex_chat_channel_1: implicit (no stored SQL)
...
-- index sqlite_autoindex_chat_channel_member_1: implicit (no stored SQL)
...
-- index sqlite_autoindex_chat_message_1: implicit (no stored SQL)
...
-- index sqlite_autoindex_discount_program_1: implicit (no stored SQL)
CREATE INDEX "JobRun_job_type_created_idx" ON "job_run" ("job_type", "created_at");

CREATE INDEX "JobRun_status_idx" ON "job_run" ("status");

-- index sqlite_autoindex_job_run_1: implicit (no stored SQL)
...
-- index sqlite_autoindex_notification_1: implicit (no stored SQL)
CREATE INDEX "NullClawExchange_channel_idx" ON "null_claw_exchange" ("channel_id");

-- index sqlite_autoindex_null_claw_exchange_1: implicit (no stored SQL)
...
-- index sqlite_autoindex_payment_1: implicit (no stored SQL)
-- index sqlite_autoindex_payment_2: implicit (no stored SQL)
-- index sqlite_autoindex_permissions_1: implicit (no stored SQL)
-- index sqlite_autoindex_permissions_2: implicit (no stored SQL)
...
-- index sqlite_autoindex_pickup_point_1: implicit (no stored SQL)
...
-- index sqlite_autoindex_place_1: implicit (no stored SQL)
-- index sqlite_autoindex_place_2: implicit (no stored SQL)
CREATE INDEX "Posts_createdAt_idx" ON "posts" ("created_at" DESC);

CREATE INDEX "idx_posts_author" ON "posts" ("author_id");

-- index sqlite_autoindex_posts_1: implicit (no stored SQL)
...
-- index sqlite_autoindex_price_alert_1: implicit (no stored SQL)
CREATE INDEX "idx_push_device_token" ON "push_device" ("token");

CREATE INDEX "idx_push_device_user" ON "push_device" ("user_id");

CREATE UNIQUE INDEX "idx_push_device_user_token_unique" ON "push_device" ("user_id", "token");

-- index sqlite_autoindex_push_device_1: implicit (no stored SQL)
CREATE INDEX "idx_refresh_tokens_hash" ON "refresh_tokens" ("token_hash");

CREATE INDEX "idx_refresh_tokens_user" ON "refresh_tokens" ("user_id");

-- index sqlite_autoindex_refresh_tokens_1: implicit (no stored SQL)
...
-- index sqlite_autoindex_review_1: implicit (no stored SQL)
CREATE UNIQUE INDEX "role_permissions_role_perm_uniq" ON "role_permissions" ("role_id", "permission_id");

-- index sqlite_autoindex_role_permissions_1: implicit (no stored SQL)
-- index sqlite_autoindex_roles_1: implicit (no stored SQL)
-- index sqlite_autoindex_roles_2: implicit (no stored SQL)
...
-- index sqlite_autoindex_route_1: implicit (no stored SQL)
CREATE UNIQUE INDEX "idx_route_picture_route_hash_unique" ON "route_picture" ("route_id", "content_hash");

CREATE INDEX "idx_route_picture_route_sort" ON "route_picture" ("route_id", "sort_order");

-- index sqlite_autoindex_route_picture_1: implicit (no stored SQL)
...
-- index sqlite_autoindex_schedule_1: implicit (no stored SQL)
...
-- index sqlite_autoindex_schedule_point_1: implicit (no stored SQL)
CREATE UNIQUE INDEX "ScheduledJob_job_type_uq" ON "scheduled_job" ("job_type");

-- index sqlite_autoindex_scheduled_job_1: implicit (no stored SQL)
-- index sqlite_autoindex_seaql_migrations_1: implicit (no stored SQL)
...
-- index sqlite_autoindex_seat_1: implicit (no stored SQL)
...
-- index sqlite_autoindex_seat_inventory_1: implicit (no stored SQL)
-- index sqlite_autoindex_staff_presence_state_1: implicit (no stored SQL)
...
-- index sqlite_autoindex_trip_session_1: implicit (no stored SQL)
...
-- index sqlite_autoindex_user_1: implicit (no stored SQL)
-- index sqlite_autoindex_user_2: implicit (no stored SQL)
-- index sqlite_autoindex_user_roles_1: implicit (no stored SQL)
...
-- index sqlite_autoindex_user_verification_1: implicit (no stored SQL)
-- index sqlite_autoindex_vehicle_type_1: implicit (no stored SQL)
-- index sqlite_autoindex_vehicle_type_2: implicit (no stored SQL)
...
-- index sqlite_autoindex_wishlist_item_1: implicit (no stored SQL)
